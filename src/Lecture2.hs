...
data FightRes = NoEndurance | Death | Vict

dragonReward :: Dragon -> Chest a -> FightRes -> Reward a
dragonReward _ _ NoEndurance = Ending "Knight ran away"
dragonReward _ _ Death = Ending "Bad end"
dragonReward (Dragon Green _ _) (Chest gold _) Vict = Reward (dragonExp Green) (Chest gold Nothing)
dragonReward (Dragon a _ _) (Chest gold treasure) Vict = Reward (dragonExp a) (Chest gold treasure)

fightIter :: Knight -> Dragon -> Int -> FightRes
fightIter (Knight health atk end) (Dragon spec dAtk dHealth) swingCount
  | health <= 0 = Death
  | end <= 0 = NoEndurance
  | swingCount == 10 = fightIter (Knight (health - dAtk) atk (end - 1)) (Dragon spec dAtk (dHealth - dAtk)) 0
  | otherwise = fightIter (Knight health atk (end - 1)) (Dragon spec dAtk (dHealth - dAtk)) (swingCount + 1)

dragonFight :: Knight -> Dragon -> Chest a -> Reward a
dragonFight knight dragon chest = dragonReward dragon chest (fightIter knight dragon 0)
...
merge xs ys = go xs ys []
  where go [] bs cs = cs ++ bs
        go as [] cs = cs ++ as
        go as bs cs 
          | head as <= head bs = go (tail as) bs (cs ++ [head as])
          | otherwise = go as (tail bs) (cs ++ [head bs])
